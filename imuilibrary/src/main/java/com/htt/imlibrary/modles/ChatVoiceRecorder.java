...
public class ChatVoiceRecorder {
    ...
    private String voiceFileDir=null;
    ...
    public String startRecording(Context appContext) {
        if(TextUtils.isEmpty(voiceFileDir)){
            throw new RuntimeException("请设置存放音频的路径");
        }
        file = null;
        try {
            ...
            voiceFilePath = voiceFileDir + File.separator + voiceFileName;
            ...
            public void run() {
                try {
                    while (isRecording) {
                        ...
                        msg.what = recorder.getMaxAmplitude();
